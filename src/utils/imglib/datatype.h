...
public:
	...
	~inputStream();
	long read(char *str, long len);
  unsigned long gcount(void);
	long eof(void);
	long tellg(void);
  void seekg(long loc);
	void clear(void);
	long isOpen(void) const { return (fp != NULL); }
};
#define BINARY 1
...
class outputStream
{
	FILE *fp;
public:
	outputStream(char *fn);
  long write(const char *str, long size);
	outputStream& operator<<(UBYTE1 val);
	outputStream& operator<<(char *val);
	outputStream& operator<<(unsigned short val);
