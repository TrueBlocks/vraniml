...
long inputStream::read(char *str, long len)
{ 
	if (!fp)
		return 0;

	size_t nItems = fread(str, len, 1, fp);
	cnt = nItems * len;
	return nItems;
}

unsigned long inputStream::gcount(void)
{
	return cnt;
}

long inputStream::eof(void)
{
	return ((fp) ? feof(fp) : 1);
}

long inputStream::tellg(void)
{
	return ((fp) ? ftell(fp) : 0);
}

void inputStream::seekg(long loc)
{
	if (fp)
...
void inputStream::clear(void)
{
}

outputStream::outputStream(char *fn)
{
	fp = fopen(fn, "w+");
}

long outputStream::write(const char *str, long size)
{
	return ((fp) ? fwrite(str, size, 1, fp) : 0);
}

outputStream& outputStream::operator<<(UBYTE1 val)
{
...
outputStream& outputStream::operator<<(unsigned short val)
{
	write((char *)&val, sizeof(val));
	return *this;
}
...
outputStream& outputStream::operator<<(unsigned int val)
{
	write((char *)&val, sizeof(val));
	return *this;
}
