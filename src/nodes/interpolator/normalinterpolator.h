#ifndef __NORMALINTERPOLATOR_H3D
#define __NORMALINTERPOLATOR_H3D
//-------------------------------------------------------------------------
...
/*----------------------------------------------------------------------
CLASS
	vrNormalInterpolator

	This class corresponds to the NormalInterpolator node in VRML.  
	<A href="../spec/part1/nodesRef.html#NormalInterpolator">Further information</a> 
	about this class is available in any VRML related documentation.

DESCRIPTION
	<ul>This node corresponds to the NormalInterpolator node as defined in the VRML97 
	specification.  Like all Node Classes this class is better described in the 
	<A href="../spec/index.html">VRML97 Specification</a>.  In particular, at this 
	<A href="../spec/part1/nodesRef.html#NormalInterpolator">link</a>.</ul>
...
MEMBERS
----------------------------------------------------------------------*/
class LIBInterface vrNormalInterpolator : public vrInterpolator
{
protected:
	//<doc>------------------------------------------------------------
...
public:
	...
	//
	vrNormalInterpolator(void);

	//<doc>------------------------------------------------------------
	// <dd>Copy constructor.
	//
	vrNormalInterpolator(const vrNormalInterpolator& v);

	//<doc>------------------------------------------------------------
	// <dd>Destructor.
	//
	virtual ~vrNormalInterpolator(void);

	//<doc>------------------------------------------------------------
	// <dd>Equals operator.
	//
	vrNormalInterpolator& operator=(const vrNormalInterpolator& v);
	...
	//
	DECLARE_NODE(vrNormalInterpolator);
...
inline void vrNormalInterpolator::AddKeyValue(const SFVec3f& keyvalue)
{
	m_KeyValue.AddValue(keyvalue);
}

inline void vrNormalInterpolator::SetKeyValue(const MFVec3f& keyvalue)
{
	m_KeyValue.Clear();
	m_KeyValue = keyvalue;
}

inline SFInt32 vrNormalInterpolator::GetNKeyValues(void) const
{
	return m_KeyValue.GetCount();
}

inline SFVec3f vrNormalInterpolator::GetKeyValue(SFInt32 n) const
{
	return m_KeyValue[n];
}

inline const MFVec3f& vrNormalInterpolator::GetKeyValueArray(void) const
{
	return m_KeyValue;
}

// eventOut sets and gets inlines
inline void vrNormalInterpolator::AddValue(const SFVec3f& value)
{
	m_Value.AddValue(value);
}

inline void vrNormalInterpolator::SetValue(const MFVec3f& value)
{
	m_Value.Clear();
	m_Value = value;
}

inline SFInt32 vrNormalInterpolator::GetNValues(void) const
{
	return m_Value.GetCount();
}

inline SFVec3f vrNormalInterpolator::GetValue(SFInt32 n) const
{
	return m_Value[n];
}

inline const MFVec3f& vrNormalInterpolator::GetValueArray(void) const
{
	return m_Value;
}

inline void vrNormalInterpolator::Traverse(vrTraverser *t) const
{
	t->Traverse(this);
}
#endif
