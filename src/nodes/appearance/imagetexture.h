#ifndef __IMAGETEXTURE_H3D
#define __IMAGETEXTURE_H3D
//-------------------------------------------------------------------------
...
CLASS
	...
	This class corresponds to the ImageTexture node in VRML.  
	<A href="../spec/part1/nodesRef.html#ImageTexture">Further information</a> 
	about this class is available in any VRML related documentation.

DESCRIPTION
	<ul>This node corresponds to the ImageTexture node as defined in the VRML97 
	specification.  Like all Node Classes this class is better described in the 
	<A href="../spec/index.html">VRML97 Specification</a>.  In particular, at this 
	<A href="../spec/part1/nodesRef.html#ImageTexture">link</a>.</ul>
...
MEMBERS
----------------------------------------------------------------------*/
class LIBInterface vrImageTexture : public vrPixelTexture
{
protected:
	//<doc>------------------------------------------------------------
...
public:
	...
	// <dd>Copy constructor.
	//
	vrImageTexture(const vrImageTexture& v);

	//<doc>------------------------------------------------------------
	// <dd>Destructor.
	//
	virtual ~vrImageTexture(void);

	//<doc>------------------------------------------------------------
	// <dd>Equals operator.
	//
	vrImageTexture& operator=(const vrImageTexture& v);
	...
	//
	DECLARE_NODE(vrImageTexture);

};

//----------------------------------------------------------------------
// field sets and gets inlines
inline void vrImageTexture::AddOrigUrl(const SFString& origurl)
{
	m_OrigUrl.AddValue(origurl);
}

inline void vrImageTexture::SetOrigUrl(const MFString& origurl)
{
	m_OrigUrl.Clear();
	m_OrigUrl = origurl;
}

inline SFInt32 vrImageTexture::GetNOrigUrls(void) const
{
	return m_OrigUrl.GetCount();
}

inline SFString vrImageTexture::GetOrigUrl(SFInt32 n) const
{
	return m_OrigUrl[n];
}

inline const MFString& vrImageTexture::GetOrigUrlArray(void) const
{
	return m_OrigUrl;
}

// exposedField sets and gets inlines
inline void vrImageTexture::AddUrl(const SFString& url)
{
	m_Url.AddValue(url);
}

inline void vrImageTexture::SetUrl(const MFString& url)
{
	m_Url.Clear();
	m_Url = url;
}

inline SFInt32 vrImageTexture::GetNUrls(void) const
{
	return m_Url.GetCount();
}

inline SFString vrImageTexture::GetUrl(SFInt32 n) const
{
	return m_Url[n];
}

inline const MFString& vrImageTexture::GetUrlArray(void) const
{
	return m_Url;
}

inline void vrImageTexture::Traverse(vrTraverser *t) const
{
	t->Traverse(this);
}
#endif
